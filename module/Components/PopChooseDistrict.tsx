...
import { apiGetDistrict, apiGetProvinces, handleGetCategories } from '../screen/Home/HomeApi';
...
const PopChooseDistrict: React.FC<Props> = ({ visible, closed, onChoose, province }) => {
  ...
  async function handleGetDistrict() {
    let res: any = await dispatch(apiGetDistrict(province.provinceid));
    console.log(res);

    if (res.payload) {
      let tempData: any = [...res.payload];
      setDataDistrict(tempData);
    }
  }

  const dataFilter = dataDistrict.filter((e) => {
    let name = e?.name || '';
    if (textSearch === '') {
      return true;
    }
...
export default PopChooseDistrict;
