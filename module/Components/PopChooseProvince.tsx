...
import { apiGetProvinces, handleGetCategories } from '../screen/Home/HomeApi';
...
const PopChooseProvince: React.FC<Props> = ({ visible, closed, onChoose }) => {
  ...
  async function handleGetProvince() {
    let res = await dispatch(apiGetProvinces({}));
    if (res.payload) {
      let tempData: any = [...res.payload];
      setDataProvince(tempData);
    }
  }

  const dataFilter = dataProvince.filter((e) => {
    let name = e?.name || '';
    if (textSearch === '') {
      return true;
    }
          ...
          <KeyboardAvoidingView style={{ flex: 1 }}>
            ...
            <ScrollView>
              {[{ id: -1, name: 'Tất cả' }, ...dataFilter].map((item: any) => {
                return (
                  <Pressable onPress={() => onChoose(item)} key={item.id} style={styles.viewTouchable}>
...
export default PopChooseProvince;
