...
export const handleGetCategories = createAsyncThunk(
  ...
  async (data, thunkApi) => {
    try {
      console.log("API handleGetCategories: ", ApiUrl + PATH_API.CATEGORIES);
      ...
      if (res.data.success) {
        ...
        console.log("handleGetCategories: ", res);
        return [];
      }
...
export const handleGetProductByCategories = createAsyncThunk(
  ...
  async (data: number, thunkApi) => {
    try {
      let res = await axios.get(`${ApiUrl}${PATH_API.PRODUCT_IN_CATEGORIES}/${data}`, {
        headers: { 'Content-type': 'application/json' },
        params: {
          limit: 100,
        },
...
export const apiGetCommentWithProduct = createAsyncThunk(
  ...
  async (data: any, thunkApi) => {
    try {
      let res = await axios.get(`${ApiUrl}${PATH_API.GET_COMMENT_WITH_PRODUCT}/${data}`, {
        headers: { 'Content-type': 'application/json' },
        params: {},
      });
      console.log(res);
